...
def accuracy(typed, source):
    ...
    count_correct = 0
    i = 0
    while i < source_len and i < typed_len:
        if typed_words[i] == source_words[i]:
            count_correct += 1
        i += 1
    return count_correct / typed_len * 100
    # END PROBLEM 3
...
# using ChatGPT after a long time of trying
...
def time_per_word(words, timestamps_per_player):
    ...
    while i < len(tpp):
        ...
        sub_times = []
        while j < len(tpp[i]) - 1:
            sub_times.append(tpp[i][j + 1] - tpp[i][j])
            j += 1
